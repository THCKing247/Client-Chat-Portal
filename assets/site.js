...
// Chatbot functionality (site-wide)
{
  ...
  const CHAT_SESSIONS_KEY = "apex_chatbot_sessions"; // Store chat sessions for client portal
  const DEMO_CLIENT_ID = "demo-001"; // Link all chats to demo account
  ...
  // Save chat session to localStorage (linked to demo account)
  ...
  function completeChatSession() {
    if (currentChatSession && currentChatSession.status === 'active') {
      const startTime = new Date(currentChatSession.startTime);
      const endTime = new Date();
      const duration = Math.round((endTime - startTime) / 1000 / 60); // minutes
      
      currentChatSession.endTime = endTime.toISOString();
      currentChatSession.duration = duration;
      currentChatSession.status = 'completed';
      saveChatSession(currentChatSession);
  ...
  if (chatbotClose) {
    chatbotClose.addEventListener("click", () => {
      ...
      // Mark session as completed when closing
      completeChatSession();
    });
  }
  ...
  if (chatbotToggle) {
    ...
    chatbotToggle.addEventListener("click", () => {
      // Start new session when opening
      if (!currentChatSession || currentChatSession.status === 'completed') {
        currentChatSession = null; // Reset to create new session
        getCurrentChatSession();
      }
    });
  }
  
  // Track page unload to complete session
...
const Auth = {
  ...
  getChatSessionsFromStorage() {
    try {
      const CHAT_SESSIONS_KEY = "apex_chatbot_sessions";
      const sessions = JSON.parse(localStorage.getItem(CHAT_SESSIONS_KEY) || '[]');
  ...
  getClientData(clientId) {
    // Get real chat sessions from localStorage
    const storedSessions = this.getChatSessionsFromStorage();
    ...
    const mockData = {
      'demo-001': {
        ...
        chatSessions: [], // Will be populated from localStorage
        services: [
          {
            id: 'svc-001',
    ...
    // Merge stored chat sessions with mock data (for demo account)
    const DEMO_CLIENT_ID = "demo-001";
    if (clientId === DEMO_CLIENT_ID) {
      const storedSessions = this.getChatSessionsFromStorage();
      // Combine with any existing mock sessions, prioritizing stored ones
      const existingSessionIds = new Set(storedSessions.map(s => s.id));
      const mockSessions = client.chatSessions || [];
      const additionalMockSessions = mockSessions.filter(s => !existingSessionIds.has(s.id));
      client.chatSessions = [...storedSessions, ...additionalMockSessions];
      
      // Update usage stats based on real sessions
      const currentMonth = new Date().getMonth();
      const currentYear = new Date().getFullYear();
      const thisMonthSessions = storedSessions.filter(s => {
        const sessionDate = new Date(s.startTime);
        return sessionDate.getMonth() === currentMonth && sessionDate.getFullYear() === currentYear;
      });
      const lastMonthSessions = storedSessions.filter(s => {
        const sessionDate = new Date(s.startTime);
      ...
      client.usage.currentMonth = thisMonthSessions.reduce((sum, s) => sum + (s.messageCount || 0), 0);
      client.usage.lastMonth = lastMonthSessions.reduce((sum, s) => sum + (s.messageCount || 0), 0);
      client.usage.totalMessages = storedSessions.reduce((sum, s) => sum + (s.messageCount || 0), 0);
      client.usage.totalSessions = storedSessions.length;
      
      // Update billing if over limit
      if (client.usage.currentMonth > client.plan.monthlyLimit) {
        const overage = client.usage.currentMonth - client.plan.monthlyLimit;
        client.billing.overageMessages = overage;
        client.billing.overageCost = overage * client.plan.overageRate;
        client.billing.currentBill = client.billing.basePrice + client.billing.overageCost;
      }
    } else if (!client.chatSessions) {
      client.chatSessions = [];
    }
...
function renderChatSessions(clientData) {
  ...
  // Refresh client data to get latest sessions from localStorage
  const session = Auth.getSession();
  if (session) {
    const freshData = Auth.getClientData(session.clientId);
    clientData = freshData;
  }
  
  if (!clientData.chatSessions) return;
